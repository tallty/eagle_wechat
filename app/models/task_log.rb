...
class TaskLog < ActiveRecord::Base
  ...
  def get_task_logs customer_id
    ...
    result = sql.select_all("select l.task_name, (Now() - l.created_at) / 60, l.end_time - l.start_time from task_logs as l, tasks as t where l.start_time > '#{datetime}' and l.task_identifier = t.identifier and t.customer_id = #{customer_id};")
    result.rows
  end
