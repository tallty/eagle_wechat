class ReportsController < ApplicationController
	...
	def month_show
	# 借口的图表数据
		@interface_info = TotalInterface.interface_info(current_customer, params[:name], @monday, :month)
		# 调用已选借口的所有客户信息
		@api_user_infos = TotalInterface.api_user_infos(current_customer, params[:name], @monday, :month)
		# 调用总数
		@total_count = TotalInterface.total_count(@api_user_infos)
	end
